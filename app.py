...
# ==================== 側邊欄選項 ====================
symbols_input = st.sidebar.text_input("股票代號（逗號分隔）", "AAPL, TSLA, NVDA, MSFT")
symbols = [s.strip().upper() for s in symbols_input.split(",") if s.strip()]

interval_options = {
    "1分鐘": "1m", "5分鐘": "5m", "15分鐘": "15m", "1小時": "60m", "日線": "1d"
}
...
def detect_breakout(df: pd.DataFrame, support: float, resistance: float,
    ...
    if (prev2_close <= (resistance - buffer)) and (prev_close <= (resistance - buffer)) and (last_close > resistance) and vol_ok:
        msg = f"突破阻力！\n股票: <b>{symbol}</b>\n現價: <b>{last_close:.2f}</b>\n阻力: {resistance:.2f}\n成交量: {last_volume/1e6:.1f}M ({vol_ratio:.1f}x)"
...
def process_symbol(symbol: str):
    ...
    fig.add_trace(go.Bar(x=df_display.index, y=df_display["Volume"], name="成交量", marker_color="lightblue", yaxis="y2"))
    if signal:
